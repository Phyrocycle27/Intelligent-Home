...
public class ClientInitializer extends ChannelInitializer<SocketChannel> {
    ...
    public ClientInitializer(SslContext sslCtx, String host, int port) {
        ...
        HOST = host;
        PORT = port;
        log.debug(String.format("Serer's host is %s and port is %d", host, port));
    }
    ...
    protected void initChannel(SocketChannel ch) {
        ...
        pipeline.addLast(sslCtx.newHandler(ch.alloc(), HOST, PORT))
                ...
                .addLast("auth", new ChannelInboundHandlerAdapter() {
                    private final Encryption enc = new Encryption();

                    @Override
                    public void channelActive(ChannelHandlerContext ctx) {
                        log.info("Channel active");
                        log.debug(String.format("Current pipeline: %s", ch.pipeline().names().toString()));
